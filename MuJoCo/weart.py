...
class WeartConnector(object):
    ...
    def get_thumb_abduction(self, hand_id: int, finger: str):
        thimble_tracking = self._fingers[WeartConnector.get_finger_id(hand_id, finger)].thimble_tracking
        return thimble_tracking.GetAbduction()
    
    def apply_force(self, hand_id: int, finger: str, force_value: float, temperature: float, texture: int):
        finger_obj = self._fingers[WeartConnector.get_finger_id(hand_id, finger)]
        # Create object WeArtTemperature and then set its value (a constructor with changeable parameters doesn't exist in this version of sdk)
        t = WeArtTemperature()
        t.active = True
        # From 0.49 and below, cold termal feedback ; 0.5 room temperature; from 0.51 and above, warm termal feedback
        t.value = temperature
        finger_obj.touch_effect.Set(t, WeArtForce(True, force_value), WeArtTexture(True, texture))
