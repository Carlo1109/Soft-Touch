...
class MujocoXRVisualizer(Visualizer, HandPoseProvider):
    def __init__(self, mj: MujocoConnector, mirror_window = False, fullscreen = False, debug = False, samples: Optional[int] = None, fps_counter = False):
    ...
    def _init_window(self):
        ...
        self._window_size = [self._width, self._height]
        if self._fullscreen:
            self._window = glfw.create_window(*self._window_size, APP_NAME + " loading...", glfw.get_primary_monitor(), None)
        else:
        ...
        GL.glLoadIdentity()
        # Proiezione ortografica per 2D (coordinate dello schermo)
        GL.glOrtho(0, self._width // 2, 0,  self._height // 2, 0,1)
